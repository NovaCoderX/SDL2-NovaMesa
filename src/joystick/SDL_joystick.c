...
int SDL_JoystickInit(void)
{
	...
	SDL_LogDebug(SDL_LOG_CATEGORY_INPUT, "SDL_JoystickInit() - Called\n");
...
int SDL_NumJoysticks(void)
{
    ...
    SDL_LogDebug(SDL_LOG_CATEGORY_INPUT, "There are %d joystick(s) attached to the system\n", total_joysticks);
...
/* These are public API and must stay linkable even though the Amiga port
 * has no USB VID/PID data - they resolve from the GUID and return 0 /
 * SDL_JOYSTICK_TYPE_UNKNOWN (or GAMECONTROLLER when a mapping exists). */
